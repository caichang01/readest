-- Fork migration 021 (upstream 020): reading-statistics archive (history tiered out of
...
-- Explicit grants for self-hosted Pigsty installations without Supabase's
-- table default privileges. Only manifests are readable by their owner.
REVOKE ALL ON public.stat_archives FROM PUBLIC, anon, authenticated;
GRANT SELECT ON public.stat_archives TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON
  public.stat_archives, public.stat_archive_state, public.stat_archive_orphans
TO service_role;
GRANT USAGE, SELECT ON SEQUENCE public.stat_archives_id_seq TO service_role;
