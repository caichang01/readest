...
vi.mock('@/helpers/updater', async () => ({
  ...(await vi.importActual<typeof import('@/helpers/updater')>('@/helpers/updater')),
  ...
  it('contains a stable Android download failure and does not install a partial APK', async () => {
    ...
    vi.mocked(window.fetch).mockResolvedValueOnce({
      ...
      json: async () => ({
        version: '0.11.20',
        platforms: {
          'android-arm64': { url: 'https://example.com/update.apk', signature: 'signature' },
        },
      }),
    } as Response);
