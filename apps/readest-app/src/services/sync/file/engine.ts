...
export interface SyncLibraryResult {
  ...
  /** Remote-only books added to the shelf; binaries are downloaded on demand. */
  booksAdded: number;
  /** Retained for callers; library sync no longer downloads book binaries. */
      ...
      await runPool(
        ...
        async (rb) => {
          ...
          try {
            ...
            result.booksAdded += 1;
            appendDiagnosticLog('file-sync', 'remote-book-shelf-added', {
              bookHash: rb.hash,
              downloadPolicy: 'on-demand',
            });
            syncedHashes.add(rb.hash);
