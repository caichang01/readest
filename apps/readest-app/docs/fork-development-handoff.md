...
- 当前第三轮候选分支为 `codex/upstream-sync-20260902`，目标上游版本 `0.12.6`。
  尚未合并 master、发布 Release 或部署生产环境；详见 3.7.2。
...
- Next.js 16.2.11 Web/API 生产构建通过。首次沙箱内运行因 Turbopack 创建本地进程并
  ...
  页面生成。
- 本机 pre-push 原先会继承默认 Node 18，并以无上限并发启动 Vitest；前者与新版
...
验收前要求：先完成全量复跑和候选 Actions；数据库主机执行
